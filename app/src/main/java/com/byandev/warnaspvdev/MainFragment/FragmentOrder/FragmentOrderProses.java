...
public class FragmentOrderProses extends Fragment {
    ...
    private void firstLoad(){
        ...
        mApiService.orderByStatus(sharedPrefManager.getSpBranchId(), sharedPrefManager.getSpOutletId(),2, limit, 0).enqueue(new Callback<RespOrderStatus>() {
            @Override
    ...
    private void loadMore() {
        itShouldLoadMore = false;
        mApiService.orderByStatus(sharedPrefManager.getSpBranchId(), sharedPrefManager.getSpOutletId(),2, limit, offset).enqueue(new Callback<RespOrderStatus>() {
            @Override
            public void onResponse(Call<RespOrderStatus> call, Response<RespOrderStatus> response) {
                if (response.isSuccessful()) {
                    if (response.body() != null && response.body().getApiStatus() == 1) {
